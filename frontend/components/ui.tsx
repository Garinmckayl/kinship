...
const BTN: Record<BtnVariant, string> = {
  primary: "bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-400 hover:to-emerald-500 text-white shadow-[0_8px_24px_rgba(20,184,166,0.26)]",
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input {...props}
      className={`w-full bg-slate-950/35 ring-1 ring-white/10 focus:ring-2 focus:ring-teal-300/70 rounded-xl px-3 py-2 outline-none placeholder:text-slate-500 text-white transition ${props.className ?? ""}`} />
  );
}
          ...
          d === null ? <span key={i} /> : (
            <button key={i} onClick={() => onPick?.(d)}
              className={`aspect-square rounded-xl text-sm grid place-items-center relative transition ${
                selected === d ? "bg-teal-600 text-white font-bold" : "hover:bg-white/10 text-slate-200"
              }`}>
              {d}
    ...
    <div className="flex gap-1 bg-slate-950/35 ring-1 ring-white/10 rounded-2xl p-1 shadow-inner">
      {tabs.map((t) => (
        <button key={t} onClick={() => onChange(t)}
          className={`flex-1 px-3 py-2 rounded-xl text-sm font-bold transition ${active === t ? "bg-gradient-to-r from-teal-500 to-emerald-600 text-white shadow-lg" : "text-slate-300 hover:text-white hover:bg-white/5"}`}>
          {t}
        </button>
      ))}
    </div>
